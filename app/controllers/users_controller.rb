...
class UsersController < ApplicationController
  ...
  def upload
    if params[:upload_file]
      ...
      puts "Type is #{params[:upload_file].class}"
      lines = file.read.split("\n")
      header = lines[0].split(",").map {|x| x.strip.downcase}
      @header = header
      fields = ["forename","surname","email","password","group"]
      indices = fields.map {|x| header.index(x.strip)}
      @indices = indices
      if indices.include?(nil)
        ...
        @successes = ""
        @failures = ""
        (1...lines.count).each do |i|
          line = lines[i].split(",")
          user_data = {}
          fields.count.times do |j|
            field = fields[j]
            index = indices[j]
            datum = line[index].strip
            user_data[field.to_sym] = datum
          end
          user_data[:name] = [user_data[:forename],user_data[:surname]].join(' ')
          user_data[:seed] = rand(100000)
          new_user = User.new(user_data)
          if new_user.update_attributes(:forename => new_user.forename,:surname => new_user.surname, :email => new_user.email.downcase, :password => new_user.password, :password_confirmation => new_user.password)
            @successes << lines[i]
            new_user.update_attribute(:confirmed, true)
            new_user.save
            sign_in current_user
            group_name = new_user.group
            if group_name
              unless Group.find_by_name(group_name)
  ...
  def interpret(score_array)
    return 'Not started' unless score_array
    return score_array[0] ? "Complete with #{score_array[1]} reset(s)." : "#{score_array[2]} out of #{score_array[3]} answers correct so far with #{score_array[1]} reset(s)."
  end
  ...
  def show
    ...
    @score = Hash[@user.task_scores.map {|x| [x[0], x[1..-1]]}]
    @task_scores = Hash[@user.task_list.map {|x| x.id.to_i}.map {|y| [y, @score[y] ? (@score[y][0] == true ? ['Complete', @score[y][1]] : ["#{@score[y][2]} out of #{Task.find(y).question_count}", @score[y][1]]) : ['Not started', 0]]} ]
    #@task_scores = @user.task_scores.map{|x| x + [Task.find(x[0]).question_count]}
    #@task_scores = Hash[scores.map {|x| [x[0], interpret( x[1..-1] + [Task.find(x[0]).question_count])] }]
    @style = Hash[@score.map {|x| [x[0], x[1] == true ? 'good' : 'bad']}]
    if params[:reseed]
      @user.reseed
   ...
   count=0
          ...
          @authorhtml=@authorhtml+%Q(</tr></table><br></h10><h7a>Choose a tag and   </h7a><h8><b><INPUT type="submit" class="bobutton" value="Click here"></b></h8><h10>
            </table>
            </form></h10>)
        end
...
#params[:newcourse]=nil
        if selected_courses.count==1
          ...
          @authorhtml=@authorhtml+%Q(</tr><tr></table></small> </h10><h7a><br>Choose one or more courses and   </h7a><h8><INPUT type="submit"  class="bobutton" value="Click here"></h8>
           </table></form>)
        end
        authorcourse.each do |course|
  ...
  def create
    if signed_in?
      ...
      if @user.update_attributes(:forename => @user.forename,:surname => @user.surname, :email => @user.email.downcase, :password => @user.password, :password_confirmation => @user.password_confirmation)
  ...
  def index
    ...
    @admin=current_user if current_user  &&current_user  .admin
